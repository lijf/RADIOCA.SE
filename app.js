...
app.get('/cases/:start/:finish', function(req, res) {
    if (req.isAuthenticated()) {
        ...
        db.lrange('cases', start, end, function(err, data) {
            if (err || !data[0]) res.send('404', 404);
            var i = 0;
            var sendcases = [];
...
app.get('/case/:id/:page', function(req, res) {
    if (req.isAuthenticated()) {
        db.sismember(
            'case:' + req.params.id + ':users',
            req.getAuthDetails().user.user_id,
            function(err, editor) {
                db.hgetall("case:" + req.params.id + ":page:" + req.params.page,
                    function(err, theCase) {
...
app.get('/case/:id/:page/edit', function(req, res) {
    if (req.isAuthenticated()) {
        db.hgetall('case:' + req.params.id + ':page:' + req.params.page, function(err, data) {
            ...
            db.sismember('case:' + req.params.id + ':users', req.getAuthDetails().user.user_id,
                function(err, editor) {
                    if (editor) {
                        res.render('edit', {
                            title: 'edit',
...
app.post('/image/:id/:page', function(req, res) {
    ...
    if (req.isAuthenticated()) requestHandlers.postImage2(req, res, db);
